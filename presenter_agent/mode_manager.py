...
class ModeManager:
    ...
    def __init__(self, screen_share, watcher, room_id: str, redis_url: str, url: str, room=None):
        self.screen_share = screen_share
        self.watcher = watcher  # InstructorScreenWatcher
        ...
        self._browser_tools = create_browser_tools(screen_share, room_id, redis_url, watcher=watcher)
        self._browser_tools_no_watcher = create_browser_tools(screen_share, room_id, redis_url)
    ...
    def get_demo_expert_tools(self) -> list:
        """Return tools for Demo Expert Mode: browser tools (no watcher) + switch to student."""
        return [
            *self._browser_tools_no_watcher,
    ...
    async def switch_to_demo(self) -> str:
        ...
        Stops watching instructor's screen and starts publishing agent's own screen.
        Returns a summary string for the tool response.
        """
        # 1. Stop watching instructor's screen share
        await self.watcher.stop()

        # 2. Generate roadmap from learnings + research
        ...
        # 4. Build new instructions and tools
        ...
        # 7. Navigate to home page and start publishing agent's screen share
        try:
    ...
    async def switch_to_student(self) -> str:
        ...
        Stops publishing agent's screen and resumes watching instructor's screen.
        Updates instructions and tools. Sets feedback flag.
        Returns a prompt string for the tool response.
        """
        # 1. Stop publishing agent's screen share
        await self.screen_share.stop_publishing()

        # 2. Resume watching instructor's screen share
